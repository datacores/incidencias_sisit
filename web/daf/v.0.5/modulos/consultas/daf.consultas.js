...
(function(API){
	...
	Consulta.prototype.ejecutar_consulta = function(){ 
		this.ejecutar_callback(this.get_consulta_xFilas());
	}
			...
			.append(
				...
				.append(
		        	...
		        	.append(
		                ...
		                $('<button>', { id: "reset_consulta", text: 'Reset', title: "Resetea listado y consultas, para crear una nueva consulta o elegir otra." })
		        	)
				),
	...
	Consulta.prototype.carga_consulta = function(q){
		...
		var columnas = q.template,
			that = this;
		
		$.each(columnas, function(k){
			k++;
			if(this.campo === "") return false; 

			that.set_campo(k, this.campo);
			that.set_ver(k, this.chekbox);
			that.set_orden(k, this.orden);
			that.set_celda(4, k, this.criterio);
			that.set_celda(5, k, this.o1);
			that.set_celda(6, k, this.o2);
			that.set_celda(7, k, this.o3);
			that.set_celda(8, k, this.o4);
			that.set_celda(9, k, this.o5);
			that.set_celda(10, k, this.o6);
			that.set_celda(11, k, this.o7);
		});
	}
			...
			API.peticion('GET', that.url+"s", false, 'json',
				function(consultas){
					consultas.forEach(function(c){
						...
						/* Limpia las templates de columnas vacias,
						 * no será necesario cuando grabe sin columnas vacias
						 */
						c.template = c.template.filter(function(t){
							return t.campo && t.campo.length >0;
						});
					});
	...
	Consulta.prototype.borra_consulta = function(nombre){
		
		if(parseInt(nombre) === -1) return false;
