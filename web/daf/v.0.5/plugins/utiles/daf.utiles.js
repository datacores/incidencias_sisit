...
(function(CORE){
     ...
     *
     * PTE REFACTORIZAR CON fecha_2obj
     */
    CORE.get_fecha = function(fecha, obj){
        ...
        if(obj_fecha == 'Invalid Date') return fecha;
        else {
            if(obj) return fecha;
    ...
    /*
     * Este metodo convierte cualquier fecha, español/ingles a objeto
     */
    CORE.fecha_2obj = function(fecha){
        if(CORE.validar('fecha_en', fecha)){
            var tmp = fecha.split("-");
            if(tmp.length < 3) tmp = fecha.split('/');
            return (tmp.length == 3) ? new Date(tmp[2]+"-"+tmp[1]+"-"+tmp[0]) : false;
        } else if(CORE.validar('fecha_sp', fecha)){
            var tmp = fecha.split("-");
            if(tmp.length < 3) tmp = fecha.split('/');
            return (tmp.length == 3) ? new Date(tmp[2]+"-"+tmp[1]+"-"+tmp[0]) : false;
        } else return false;
    ...
    /**
     * Codigo obtenido del que genera typescript, para extender en la herencia de clases.
     * Modificamos algunas partes para adaptarlo a nuestras necesidades.
     * 
     * @param  {Class} d Clase hija
     * @param  {Class} b Clase padre
     * @return No retorna nada porque deja modificada la clase hija.
     */
    CORE.extends = function(d, b) {
