 ...
 *  
 *  Para el futuro, molaria la generacion de modales con la sintaxis directa y simple de toastr
 */
...
(function(){
	...
	/*
	 * Para la ejecución de la aplicación y ejecuta el callback reanudando la ejecución
	 * tras pulsar un botón.
	 */
	Confirmar = function(msj, _true, _false, callback){
		
		var _true  = _true  || 'OK';
			_false = _false || 'Cancel',
			callback = _.isFunction(callback) ? callback : function(r){ return r; };

		// width, height, close, left, top, padding, color_fondo, borde, radio_borde, sombra
		var confirmar = new Modal('450px', '200px');
		confirmar.set_html('<p style="margin-top:11px;">'+msj+'</p><div style="position: absolute;bottom: 11px;left: 20px;width: 400px;"><hr style="margin-bottom:11px;" /><button id="ok" style="float:right;">'+_true+'</button><button id="cancel" style="float:right;margin-right: 7px;">'+_false+'</button></div>');
		confirmar.abre_modal(function(modal){
			var $modal = $(modal);
			
			$modal.find('button').off('click');
			$modal.find('button').on('click', function(){ 
				confirmar.cierra_modal(); 
				var r = $(this).attr('id') == 'ok' ? true : false;
				callback(r);
			});
		});
	}
