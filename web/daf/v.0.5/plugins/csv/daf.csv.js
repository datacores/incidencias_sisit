(function(CORE){
     ...
     * Si es un array de arrays, todos los arrays del array debe de tener el mismo numero de elementos.
     * Parametros:
     ...
     * titulos  - Objeto para crear/formatear los titulos
    ...
    CSV.prototype.set_datos = function(datos_recibidos){
        ...
        if(_.isObject(datos[0])){
            var claves  = _.keys(datos[0]);
                titulos = _.clone(claves);
            
            if(this.titulos){
                var i;
                _.each(this.titulos, function(_titulo, _key){
                    i = titulos.indexOf(_key);
                    if(i > -1) titulos[i] = _titulo;
                });
            }

            this.datos = [_.object(claves, titulos)];
