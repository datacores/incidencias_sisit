 ...
 * 
 * QUE TODAS LOS TD SE LEA COMO SI FUERA UN TEMPLATE UNDERSCORE, COMO ACTUALMENTE LO HACE DATA-CODIGO. SI ES EDITABLE LO META COMO CONTENIDO DE UN INPUT.
 * SI NO ES EDITABLE LO META COMO CONTENIDO DE UN SPAN. DE ESTA MANERA EL DATA-ID, SOLO ES EL CAMPO SOBRE EL QUE ACTUA
 * EL FILTRO Y REORDENAR.
 * 
 */
...
(function(CORE){
		...
		switch(tipo){
			...
			case 'select':
				...
				$.each($(val_celda[0]).find('option'), function() {
					valor_option = (_.isBoolean(v)) ? !!this.value : this.value;
					select += '<option data-valor="<%='+v+'%>" value="'+valor_option+'"<% if('+v+' == "'+valor_option+'"){ print("selected"); } %> >'+this.innerHTML+'</option>';	
				});
			...
			case 'html':
				...
				$.each($objs_w_cdgo, function(){
					if($(this).attr('data-codigo')){
						...
						$(this).html('\
							<span identificador="<%=_identificador%>" fila="<%=_fila_registro%>" columna="'+columna+'" style="width:100%">\
							<%'+codigo+'%>\
							</span>');
					}
				});
	...
	Tabla.prototype.carga_registros = function(registros, campo_id, callback){
		...
		// Cargamos filas de operaciones por columnas si las hubiera
