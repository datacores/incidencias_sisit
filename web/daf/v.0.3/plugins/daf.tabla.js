(function(CORE){
	...
	Tabla.prototype.get_celda = function(tipo, val_celda, columna){
		...
		switch(tipo){
			...
			case 'html':
				...
				/*
				 * Recoge el codigo de data-codigo="codigo a insertar" y lo inserta
				 * en el elemento que tenga el atributo data-codigo.
				 */
	...
	Tabla.prototype.comportamientos = function(){
		...
		this.$cuerpo.on('change', 'input', function(){ 
			...
			var obj_datos = {
				columna: 			  $(that.$tabla.find('thead tr:nth-child(1) th')[columna_num]).attr('data-id'),
				valor_anterior: 	  that.valor_celda_focus,
				valor_actual: 		  $(this).val()
			}

			obj_datos[that.identificador] =  $(this).attr('identificador');

			// El if evita que actue sobre filtros
			if(!isNaN(parseInt($(this).attr('fila')))){
				var columna_num = $(this).attr('columna');
				that.cambia_celda(
					this, // Celda ( del DOM)
					obj_datos,
					$(this).attr('fila'), // fila
		...
		this.$cuerpo.on('change', 'select', function(){ that.cambia_celda(this); });
