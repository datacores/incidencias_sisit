...
(function(CORE){
	...
	Filtro.prototype.add_condicion = function(){
		...
		if(tipo.toLowerCase() === 'string'){
			var condicion = new Condicion(campo, operador, expresion, id);
			procesa_condicion(condicion);
		} else if(tipo.toLowerCase() === 'array'){
			var condicion;
			_.each(campo, function(c){
	...
	CORE.aplicar_filtro = function(filtro, datos){
		if(_.isArray(datos)){
			...
			if(!typeof datos || !_.isArray(datos) || !datos.length > 0 || !_.isObject(datos[0]))
	 ...
	 */
	CORE.aplicar_filtros = function(filtros, datos, union){
